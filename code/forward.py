...
def forward(self,inputs):
    ...
    def _convModel(name,inputs,weights):
        ...
        """
        # 使用正态分布初始化核
        kernel = tf.get_variable(name , [weights[0], weights[0], weights[1], weights[2]], dtype=tf.float32,
                                 initializer=tf.truncated_normal_initializer(stddev=0.01))
        # 0初始化bias
        bias = tf.get_variable(name + "B", [weights[2]], initializer=tf.constant_initializer(0.))
        input = tf.nn.conv2d(inputs, kernel, [1, weights[3], weights[3], 1], "SAME") + bias
        relu=tf.nn.relu(tf.nn.conv2d(inputs, kernel, [1, weights[3], weights[3], 1], "SAME") + bias)
    ...
    #卷积1
    conv1 = _convModel("CONV1", inputs, [3, 1, 32, 2])
    #卷积2
    conv2 = _convModel("CONV2", conv1, [3, 32, 64, 2])
    #卷积3
    conv3 = _convModel("CONV3", conv2, [3, 64, 128, 2])
...
class TrainModel(object):
    def __init__(self, train_img_path, verify_img_path, char_set, model_save_dir, verify=False):
        ...
        if captcha_shape_len == 3:
            ...
            raise TrainException("图片转换为矩阵时出错，请检查图片格式")
    ...
    def confirm_image_suffix(self):
        ...
        for index, img_name in enumerate(self.train_images_list):
            ...
            if not img_name.endswith(sample_conf['image_suffix']):
                raise TrainException('confirm images suffix：you request [.{}] file but get file [{}]'
